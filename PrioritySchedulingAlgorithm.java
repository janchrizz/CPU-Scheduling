/** PrioritySchedulingAlgorithm.java
 *
 * A single-queue priority scheduling algorithm.
 *
 * @author: Charles Zhu
 * Spring 2016
 *
 */
package com.jimweller.cpuscheduler;

import java.util.*;

import com.jimweller.cpuscheduler.Process;

public class PrioritySchedulingAlgorithm extends BaseSchedulingAlgorithm implements OptionallyPreemptiveSchedulingAlgorithm {
    private Vector<Process> jobs;
    boolean x;
    
    PrioritySchedulingAlgorithm(){
        x = false;
        activeJob = null;
        jobs = new Vector<Process>();
        
    }
    
    /** Add the new job to the correct queue.*/
    public void addJob(Process p){
        
        jobs.add(p);
        
    }
    
    /** Returns true if the job was present and was removed. */
    public boolean removeJob(Process p){
        return jobs.remove(p);
    }
    
    /** Transfer all the jobs in the queue of a SchedulingAlgorithm to another, such as
     when switching to another algorithm in the GUI */
    public void transferJobsTo(SchedulingAlgorithm otherAlg) {
        throw new UnsupportedOperationException();
    }
    
    
    /** Returns the next process that should be run by the CPU, null if none available.*/
    public Process getNextJob(long currentTime)
    {
        if(x==false)
        {
            activeJob = jobs.get(0);
            if(jobs.get(0).isActive())
                return activeJob;
            
            if(jobs.size()==0)
                return null;
            else
            {
                for(int i = 1; i < jobs.size(); i++)
                {
                    if(jobs.get(i).isActive())
                        return jobs.get(i);
                    else if(jobs.get(i).getPriorityWeight() < activeJob.getPriorityWeight() && jobs.get(i).getArrivalTime() <= currentTime)
                        activeJob = jobs.get(i);
                    else if(jobs.get(i).getPriorityWeight() == activeJob.getPriorityWeight() && jobs.get(i).getArrivalTime() <= currentTime)
                    {
                        if(jobs.get(i).getPID()<activeJob.getPID())
                            activeJob = jobs.get(i);
                    }
                }
                return activeJob;
            }
        }
        
        else
        {
            if(jobs.size()==0)
                return null;
            else
            {
                activeJob = jobs.get(0);
                for(int i = 1;i<jobs.size();i++)
                {
                    if(jobs.get(i).isActive())
                        activeJob = jobs.get(i);
                }
                for(int i=1;i<jobs.size();i++)
                {
                    if(jobs.get(i).getPriorityWeight() < activeJob.getPriorityWeight() && jobs.get(i).getArrivalTime() <= currentTime)
                    {
                        activeJob = jobs.get(i);
                    }
                    else if(jobs.get(i).getPriorityWeight() == activeJob.getPriorityWeight() && jobs.get(i).getArrivalTime() <= currentTime)
                    {
                        if(jobs.get(i).getPID()<activeJob.getPID())
                            activeJob = jobs.get(i);
                    }
                }
                return activeJob;
            }
        }
    }
    
        public String getName(){
            return "Single-Queue Priority";
        }
        
        /**
         * @return Value of preemptive.
         */
        public boolean isPreemptive(){
            return x;
        }
        
        /**
         * @param v Value to assign to preemptive.
         */
        public void setPreemptive(boolean v){
            x = v;
            
        }
        
    }